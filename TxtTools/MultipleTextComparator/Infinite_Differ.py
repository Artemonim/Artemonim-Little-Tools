#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Multiple Text Comparator: A tool for comparing multiple text files simultaneously.
...
Example:
    Run the application with:
        $ python Infinite_Differ.py
"""
# TODO: remove unnecessary comments
# TODO: remove outdated GUI elements and code
# TODO: update processing rules in initial Docstring according to program code
...
CACHE_FILENAME = os.path.join(SCRIPT_DIR, "infinite_differ_cache.pkl")
COLORS_FILENAME = os.path.join(SCRIPT_DIR, "infinite_differ_colors.json")
...
To get started:
1. Add files (not tested) or text sources using the toolbar buttons
2. View the differences highlighted with colors in auto tab
3. Select custom groups of sources to compare
"""

# * Default source tab colors
...
class LineNumberArea(QWidget):
    ...
    Provides a visual representation of line numbers alongside a corresponding QTextEdit.
    This class handles the drawing of line numbers, and ensures correct alignment and
    sizing relative to the associated text editor.

    Args:
        editor (QTextEdit): The text editor widget for which line numbers are displayed.
    """
    ...
    def sizeHint(self):
        ...
        Returns:
            QSize: The recommended size for the line number area.
        """
...
class SearchWidget(QWidget):
    ...
    Provides a user interface for searching text within a QTextEdit, including
    features for searching up, down, counting occurrences, and highlighting matches.

    Args:
        text_edit (QTextEdit): The text edit widget in which to perform searches.
    ...
    def count_matches(self):
        ...
        Counts the occurrences of the search term and displays the count
        either in the main window's status bar (if available) or as a tooltip.
        """
        search_text = self.search_input.text()
        if not search_text:
            return
...
class DiffTextEdit(QTextEdit):
    ...
    Extends QTextEdit to include features such as line numbers, search functionality,
    and context-aware highlighting for diff operations.
    ...
    def update_line_number_area(self, rect=None, dy=0):
        ...
        Args:
            rect (QRect, optional): The update rectangle. Defaults to None.
            dy (int, optional): The vertical scroll value. Defaults to 0.
        """
        if dy:
...
class TextSource:
    ...
    Attributes:
        name (str): Display name of the text source.
        content (str): String content of the text source.
        lines (List[str]): List of lines parsed from the content.
        color (QColor): QColor assigned to this text source for highlighting.
    """
...
class SourceSelectionDialog(QDialog):
    ...
    Allows the user to select multiple text sources from a list for comparison.

    Args:
        sources (List[TextSource]): List of available text sources.
        parent (QWidget, optional): The parent widget. Defaults to None.
    """
...
class MultiDiffView(QWidget):
    ...
    Displays the results of a multi-source diff, highlighting differences
    and similarities between the selected text sources.

    Args:
        diff_manager (DiffManager): The diff manager instance.
        source_indices (List[int]): List of indices of text sources to compare.
        parent (QWidget, optional): The parent widget. Defaults to None.
    """
...
class ColorButton(QPushButton):
    ...
    Provides a button that displays a selected color and allows the user to
    change the color using a color picker dialog.

    Args:
        color (QColor, optional): Initial color. Defaults to white.
        parent (QWidget, optional): The parent widget. Defaults to None.
    """
...
class WelcomeWidget(QWidget):
    ...
    Presents a welcome message to the user when the application starts
    or when no tabs are currently open.
...
class InfiniteDifferApp(QMainWindow):
    ...
    Provides the main user interface for the InfiniteDiffer application,
    including features for adding, comparing, and managing multiple text sources.
    """
