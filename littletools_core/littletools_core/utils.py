...
"""
    ...
    Classes/Functions:
    - BatchTimeEstimator (line 303):
      - start() -> None (line 312)
      - add_item(workload: float) -> None (line 316)
      - update(workload_done: float) -> None (line 321)
      - get_eta_str() -> str (line 327)
      - Functions:
        - is_interrupted() -> bool (line 69)
        - register_cleanup_function(func: Callable[[], None]) -> None (line 74)
        - _signal_handler(signum: int, frame: Any) -> None (line 80)
        - setup_signal_handler() -> None (line 97)
        - safe_delete(file_path: Union[str, Path]) -> bool (line 102)
        - ensure_dir_exists(dir_path: Union[str, Path]) -> bool (line 117)
        - clean_partial_output(output_path: Union[str, Path], max_attempts: int = 3) -> bool (line 164)
        - print_separator(char: str = "═", length: int = 50) -> None (line 184)
        - print_status(message: str, status_type: str = "info") -> None (line 197)
        - clear_screen_if_compact(is_compact: bool) -> None (line 209)
        - format_duration(seconds: float) -> str (line 215)
