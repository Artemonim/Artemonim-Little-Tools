...
from littletools_core.utils import (BatchTimeEstimator, ensure_dir_exists,
                                   format_duration, setup_signal_handler)
...
async def process_file(
    ...
    if not overwrite and output_path.exists():
        console.print(f"[{position}/{total}] Skipping {file_path.name} (output exists).")
    ...
    success = await run_ffmpeg_command(cmd, stats, quiet=True, output_path=str(output_path))
    if success:
...
@app.command()
def run(
    input_dir: Annotated[Path, typer.Option("--input", "-i", help="Input directory with MKV files.")] = INPUT_DIR,
    output_dir: Annotated[Path, typer.Option("--output", "-o", help="Output directory for normalized files.")] = OUTPUT_DIR,
    overwrite: Annotated[bool, typer.Option(help="Overwrite existing files.")] = False,
    concurrency: Annotated[int, typer.Option(help="Number of files to process at once.")] = 2,
):
    """
