...
async def create_video(image_path: Path, audio_path: Path, output_path: Path):
    ...
    try:
        with Image.open(image_path) as img:
            ...
            if width % 2 != 0 or height % 2 != 0:
                console.print("[yellow]! Warning: Image dimensions are not even. This may cause issues with some video codecs.[/yellow]")
    ...
    if success:
        console.print(f"[green]✓ Video created successfully![/green] Output: {output_path.name}")
    else:
        console.print(f"[red]! Video creation failed. Check FFmpeg output for details.[/red]")
        raise typer.Exit(code=1)


@app.command()
def run(
    image_file: Annotated[Path, typer.Option("--image", "-i", help="Path to the input image file.")],
    audio_file: Annotated[Path, typer.Option("--audio", "-a", help="Path to the input audio file.")],
    output_file: Annotated[Optional[Path], typer.Option("--output", "-o", help="Path for the output video file. [default: <image_stem>.mp4]")] = None,
):
    """
