...
def sam_refine(
    ...
    """
    print(
        f"[DEBUG] sam_refine called with click at ({evt.index[0]}, {evt.index[1]}), prompt: {point_prompt}"
    )
    ...
    print(
        f"[DEBUG] sam_refine completed successfully, returning painted_image shape: {painted_image.shape}"
    )
...
# video matting
async def video_matting(
    video_state, interactive_state, mask_dropdown, erode_kernel_size, dilate_kernel_size
):
...
def add_audio_to_video(video_path, audio_path, output_path):
    try:
        ...
        _ = ffmpeg.output(
            video_input, audio_input, output_path, vcodec="copy", acodec="aac"
...
def generate_video_from_frames(
    frames, output_path, fps=30, gray2rgb=False, audio_path=""
):
    """
...
sam_checkpoint = load_file_from_url(
    sam_checkpoint_url_dict[args.sam_model_type], checkpoint_folder
)
# initialize sams
...
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/test-sample0-720p.mp4",
    test_sample_path,
)
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/test-sample1-720p.mp4",
    test_sample_path,
)
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/test-sample2-720p.mp4",
    test_sample_path,
)
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/test-sample3-720p.mp4",
    test_sample_path,
)
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/test-sample0.jpg",
    test_sample_path,
)
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/test-sample1.jpg",
    test_sample_path,
)
...
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/tutorial_single_target.mp4",
    assets_path,
)
load_file_from_url(
    "https://github.com/pq-yang/MatAnyone/releases/download/media/tutorial_multi_targets.mp4",
    assets_path,
)
...
with gr.Blocks(theme=gr.themes.Monochrome(), css=my_custom_css) as demo:
    ...
    with gr.Group(elem_classes="gr-monochrome-group", visible=True):
        with gr.Row():
            with gr.Accordion(
                "📕 Video Tutorial (click to expand)",
                open=False,
                elem_classes="custom-bg",
            ):
                with gr.Row():
                    with gr.Column():
                        gr.Markdown("### Case 1: Single Target")
                        gr.Video(
                            value="./assets/tutorial_single_target.mp4",
                            elem_classes="video",
                        )
                    ...
                    with gr.Column():
                        ...
                        gr.Video(
                            value="./assets/tutorial_multi_targets.mp4",
                            elem_classes="video",
                        )
    ...
    with gr.Tabs():
        with gr.TabItem("Video"):
            ...
            with gr.Group(elem_classes="gr-monochrome-group", visible=True):
                with gr.Row():
                    with gr.Accordion(
                        "MatAnyone Settings (click to expand)", open=False
                    ):
                        with gr.Row():
                        ...
                        with gr.Row():
                            image_selection_slider = gr.Slider(
                                ...
                                value=1,
                                label="Start Frame",
                                info="Choose the start frame for target assignment and video matting",
                                visible=False,
                            )
                ...
                with gr.Row(equal_height=True):
                    ...
                    with gr.Column(scale=2):
                        step2_title = gr.Markdown(
                            "## Step2: Add masks <small>(Several clicks then **`Add Mask`** <u>one by one</u>)</small>",
                            visible=False,
                        )
                with gr.Row(equal_height=True):
                    with gr.Column(scale=2):
                        video_input = gr.Video(
                            label="Input Video", elem_classes="video"
                        )
                        extract_frames_button = gr.Button(
                            value="Load Video",
                            interactive=True,
                            elem_classes="new_button",
                        )
                    ...
                    with gr.Column(scale=2):
                        ...
                        template_frame = gr.Image(
                            ...
                            type="pil",
                            interactive=True,
                            elem_id="template_frame_video",
                            visible=False,
                            elem_classes="image",
                        )
                        with gr.Row(
                            equal_height=True, elem_classes="mask_button_group"
                        ):
                            ...
                            matting_button = gr.Button(
                                ...
                                interactive=True,
                                visible=False,
                                elem_classes="green_button",
                                min_width=100,
                            )

                gr.HTML(
                    '<hr style="border: none; height: 1.5px; background: linear-gradient(to right, #a566b4, #74a781);margin: 5px 0;">'
                )
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=2):
                        foreground_video_output = gr.Video(
                            label="Foreground Output",
                            visible=False,
                            elem_classes="video",
                        )
                        foreground_output_button = gr.Button(
                            value="Foreground Output",
                            visible=False,
                            elem_classes="new_button",
                        )
                    with gr.Column(scale=2):
                        alpha_video_output = gr.Video(
                            label="Alpha Output", visible=False, elem_classes="video"
                        )
                        alpha_output_button = gr.Button(
                            value="Alpha Mask Output",
                            visible=False,
                            elem_classes="new_button",
                        )
            ...
            def get_frames_from_video_scoped(video_input, video_state):
                ...
                """
                print(
                    f"[DEBUG] get_frames_from_video_scoped called with video: {video_input}"
                )
                ...
                compatible_video_path = asyncio.run(
                    convert_to_compatible_mp4(video_input, output_dir="temp_videos")
                )
                if not compatible_video_path:
                    raise gr.Error(
                        "Video conversion failed. Please try another video format."
                    )
                video_path = compatible_video_path
                conversion_message = (
                    f"Converted and resized to 720p: {Path(video_path).name}"
                )
                ...
                try:
                    ...
                    ffmpeg.input(video_path).output(
                        audio_path, format="wav", acodec="pcm_s16le", ac=2, ar="44100"
                ...
                video_state = {
                    ...
                    "masks": [
                        np.zeros((frames[0].shape[0], frames[0].shape[1]), np.uint8)
                    ]
                    * len(frames),
                ...
                return (
                    ...
                    gr.update(
                        value=video_state["origin_images"][0], visible=True
                    ),  # template_frame
                    gr.update(
                        visible=True, maximum=len(frames), value=1
                    ),  # image_selection_slider
                    gr.update(
                        visible=False, maximum=len(frames), value=len(frames)
            ...
            track_pause_number_slider.release(
                ...
                outputs=[template_frame, interactive_state],
                api_name="end_image",
            )
            ...
            gr.Examples(
                examples=[
                    os.path.join(
                        os.path.dirname(__file__), "./test_sample/", test_sample
                    )
                    for test_sample in [
                        "test-sample0-720p.mp4",
                        "test-sample1-720p.mp4",
                        "test-sample2-720p.mp4",
                        "test-sample3-720p.mp4",
                    ]
                ],
                inputs=[video_input],
            )
        ...
        with gr.TabItem("Image"):
            ...
            with gr.Column():
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=2):
                        image_input = gr.Image(
                            label="Input Image", elem_classes="image"
                        )
                        extract_frames_button = gr.Button(
                            value="Load Image",
                            interactive=True,
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=2):
                        foreground_image_output = gr.Image(
                            type="pil",
                            label="Foreground Output",
                            visible=False,
                            elem_classes="image",
            ...
            def get_frames_from_image_scoped(image_input, image_state):
                print(
                    f"[DEBUG] get_frames_from_image_scoped called with image shape: {image_input.shape if image_input is not None else 'None'}"
                )
                user_name = time.time()
                ...
                return (
                    ...
                    gr.update(
                        value=image_state["origin_images"][0], visible=True
                    ),  # template_frame
                    gr.update(
                        label="Num of Refinement Iterations",
                        info="More iterations → More details & More time",
            ...
            gr.Examples(
                examples=[
                    ...
                    for test_sample in ["test-sample0.jpg", "test-sample1.jpg"]
                ],
                inputs=[image_input],
            )
