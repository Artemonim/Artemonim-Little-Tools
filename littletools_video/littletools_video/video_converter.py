...
"""
    ...
    Classes/Functions:
        - _process_single_file_for_conversion(file_path: Path, output_dir: Path, quality: str, fps: str, resolution: str, normalize_audio: bool, overwrite: bool, codec: str, stats: ProcessingStats, estimator: BatchTimeEstimator, position: int, total: int, use_original_name: bool = False, output_filename: Optional[str] = None) -> None (line 63)
...
def _get_cq40_scale_filter() -> str:
    """Return scale filter for CQ=40 (<=720p) with commas escaped for FFmpeg."""
    # * Escapes commas to prevent filterchain splitting by FFmpeg parser
...
async def _process_single_file_for_conversion(
    ...
    # * apply the legacy 720p scaling behavior.
        ...
        # * Use escaped commas so FFmpeg does not split the filter incorrectly
        filters.append(_get_cq40_scale_filter())
        apply_lanczos = True
    ...
    if apply_lanczos:
        video_cmd.extend(["-sws_flags", "lanczos"])
    if filters:
