...
"""
    ...
    Classes/Functions:
        - _process_single_file_for_conversion(file_path: Path, output_dir: Path, quality: str, fps: str, resolution: str, normalize_audio: bool, overwrite: bool, codec: str, stats: ProcessingStats, estimator: BatchTimeEstimator, position: int, total: int, use_original_name: bool = False) -> None (line 63)
        - convert(input_dir: Annotated[Path, typer.Option('--input', '-i', help='Input directory containing videos.')] = INPUT_DIR, output_dir: Annotated[Path, typer.Option('--output', '-o', help='Output directory for converted videos.')] = OUTPUT_DIR, quality: Annotated[str, typer.Option(help='Encoding quality (CQ value). [26|30|34|40]')] = '26', fps: Annotated[str, typer.Option(help='Target FPS. [original|30|60|120]')] = 'original', resolution: Annotated[str, typer.Option(help='Target resolution (scales down only). [original|480p|720p|1080p|2160p]')] = 'original', codec: Annotated[str, typer.Option(help='Video codec for encoding. [hevc|h264]')] = 'hevc', normalize_audio: Annotated[bool, typer.Option(help='Normalize audio to -16 LUFS.')] = False, overwrite: Annotated[bool, typer.Option(help='Overwrite existing files in the output directory.')] = False, concurrency: Annotated[int, typer.Option(help='Number of files to process concurrently.')] = 2) -> None (line 170)
        - single() -> None (line 294)
        - tree() -> None (line 504)
        - _convert_single_file_for_merge(file_path: Path, temp_dir: Path, quality: str, fps: str, resolution: str, normalize_audio: bool, codec: str, stats: ProcessingStats, estimator: BatchTimeEstimator, position: int, total: int) -> Optional[Path] (line 802)
        - merge(inputs: Annotated[List[Path], typer.Option('--input', '-i', help='Input video file. Provide this option multiple times for each file in order.')], output: Annotated[Path, typer.Option('--output', '-o', help='Output file path for the merged video.')], quality: Annotated[str, typer.Option(help='Encoding quality for intermediate files. [26|30|34|40]')] = '26', fps: Annotated[str, typer.Option(help='Target FPS for intermediate files. [original|30|60|120]')] = 'original', resolution: Annotated[str, typer.Option(help='Target resolution for intermediate files. [original|480p|720p|1080p|2160p]')] = 'original', codec: Annotated[str, typer.Option(help='Video codec for intermediate conversion. [hevc|h264]')] = 'hevc', normalize_audio: Annotated[bool, typer.Option(help='Normalize audio during intermediate conversion.')] = False, cleanup: Annotated[bool, typer.Option('--cleanup-source', help='Delete source files upon successful merge.')] = False) -> None (line 879)
        - compile(inputs: Annotated[List[Path], typer.Option('--input', '-i', help='Input files. First is video source. Provide option multiple times.')], output_dir: Annotated[Path, typer.Option('--output', '-o', help='Output directory for the compiled file.')] = OUTPUT_DIR, keep_original_audio: Annotated[bool, typer.Option(help='Keep audio from the primary video source file.')] = False, keep_original_subtitles: Annotated[bool, typer.Option(help='Keep subtitles from the primary video source file.')] = False, output_container: Annotated[str, typer.Option(help='Container for the output file. [mkv|mp4]')] = 'mkv', overwrite: Annotated[bool, typer.Option(help='Overwrite existing file in the output directory.')] = False) -> None (line 1036)
...
def convert(
    ...
    delete_source: Annotated[
        str,
        typer.Option(
            help="Delete source files after successful conversion. [no|yes|ask]"
        ),
    ] = "no",
) -> None:
    """
    ...
    async def main_async_logic() -> float:
        ...
        # After all tasks, if delete_source == "ask", prompt user
        if delete_source == "ask":
            if typer.confirm("Delete source files for successfully converted videos?"):
...
def single() -> None:  # noqa: C901
    ...
    try:
        ...
        async def run_conversion() -> None:
            ...
            for idx, file_path in enumerate(files_to_process, 1):
                try:
                    ...
                    result = await _process_single_file_for_conversion(
                        file_path=file_path,
                        output_dir=out_dir,
                        ...
                        total=total_files,
                        use_original_name=False,
                        delete_source=final_settings["delete_source"],
                    )
                    if result:
        ...
        # After all conversions, if delete_source == "ask", prompt user
        if final_settings["delete_source"] == "ask":
            if typer.confirm("Delete source files for successfully converted videos?"):
                for f in successful_files:
                    safe_delete(f)
    ...
    try:
        ...
        async def main_async_logic() -> None:
            ...
            if video_files_to_process:
                ...
                tasks: List[Coroutine[Any, Any, Optional[Path]]] = []
        ...
        # After tree processing, if delete_source != "no", remove empty directories
        if final_settings["delete_source"] != "no":
...
def remove_empty_dirs(root: Path) -> None:
    """Recursively removes empty directories from the given root."""
